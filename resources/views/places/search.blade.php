        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    <form method="GET" action="{{ route('places.search') }}" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <!-- 検索 -->
                            <div>
                                <x-input-label for="q" :value="__('検索')" />
                                <x-text-input id="q" class="block mt-1 w-full" type="text" name="q"
                                    :value="request('q')" placeholder="場所名、説明、タグで検索" />
                            </div>
            ...
            @if($searchQuery)
                <div class="mb-6">
                    <h3 class="text-lg font-medium text-gray-900">
                        "{{ $searchQuery }}" の検索結果 ({{ $places->total() }}件)
                    </h3>
                </div>
            @endif
